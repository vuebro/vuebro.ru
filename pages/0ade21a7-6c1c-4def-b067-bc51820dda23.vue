<template>
<div class="flex items-center bg-cover bg-center min-h-[66dvh]" :style="`background-image:url(${the.images[0].url})`"
    un-cloak>
        ...
        <div
            ...
            <div class="text-center lg:text-left text-xs sm:text-sm md:text-base">
                <h3 class="text-2xl lg:text-3xl text-slate-50 mb-6 font-['Rubik_Mono_One'] drop-shadow">{{
                    visible
                    }}<span class="blink underline decoration-white decoration-4 text-transparent">{{ title[length] }}</span><span class="invisible">{{ invisible }}</span>
                </h3>
