...
const messages = {
    en: {
        ...
        p4: "Examples of Connecting Modules from Popular Libraries and Frameworks to VueBro",
        link4: "modules",
        h3: "Using `importmap`",
        p5: "To add a module to the 'importmap,' use the tool from the dropdown menu in the upper right corner of the main VueBro window. The 'importmap' allows you to use names when importing modules, which is particularly useful when names are used in imports from online package modules. Additionally, all modules from the 'importmap' in VueBro are preloaded using the `preload` directive.",
        h4: "Importing Styles",
    ...
    ru: {
        ...
        p4: "Примеры подключения модулей популярных библиотек и фреймворков к VueBro",
        link4: "модули",
        h3: "Использование importmap",
        p5: "Для добавления модуля в importmap воспользуйтесь инструментом из выпадающего меню в правом верхнем углу основного окна VueBro. Importmap дает возможность использовать имена при импортировании модулей, что полезно, если имена используются при импортировании в модулях из онлайн пакетов. Также все модули из importmap в VueBro предзагружаются с помощью директивы preload.",
